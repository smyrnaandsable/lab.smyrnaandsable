export default async function handler(req, res) {
    // Sadece POST isteklerine izin veriyoruz, dışarıdan tarayıcıya yazıp kurcalamasınlar
    if (req.method !== 'POST') {
        return res.status(405).json({ error: 'Method not allowed' });
    }

    const REPO_OWNER = "ceylanbelin";
    const REPO_NAME = "lab.smyrnaandsable";
    // Vercel kasasına kilitlediğimiz şifreyi arkadan çağırıyoruz
