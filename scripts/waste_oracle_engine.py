...
def save_log(entry):
    # GitHub Action ortamında dosya yolları bazen karışabilir, mutlak yolu garantiye alalım
    base_dir = os.path.dirname(os.path.abspath(__file__))
    log_dir = os.path.join(os.path.dirname(base_dir), 'logs') # Ana dizindeki logs klasörü
...
if __name__ == "__main__":
    # GitHub Actions'tan gelen INPUT değerlerini ortam değişkenlerinden al
    # Eğer input yoksa 'Manual Audit' varsayılanını kullan
    trigger_reason = os.getenv('INPUT_REASON', 'Manual Audit')
    
    print(f"Sovereign Engine: Initializing with reason: {trigger_reason}")
    ...
    # Mantık: Eğer otomatik bir işlemse (scheduled) metrikleri randomize et
    if "scheduled" in trigger_reason.lower() or os.getenv('GITHUB_EVENT_NAME') == 'schedule':
